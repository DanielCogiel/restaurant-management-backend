...
//            order.setId(updatedOrder.getId());
            ...
            return Response.ok().entity("{\"message\":\"Zam闚ienie zosta這 zaktualizowane.\"}").build();
        } else {
            return Response.status(Response.Status.NOT_FOUND).entity("{\"message\":\"Zam闚ienie o podanym identyfikatorze nie zosta這 znalezione.\"}").build();
        }
    }
	...
	public Response delete(@PathParam("id") int id){
        try {
            orderBean.delete(id);
            return Response.ok().entity("{\"message\":\"Zam闚ienie zosta這 usuni皻e.\"}").build();
        } catch(Exception e) {
            return Response.status(Response.Status.NOT_FOUND).entity("{\"message\":\"Zam闚ienie o podanym identyfikatorze nie zosta這 znalezione.\"}").build();
        }
	}
